...
```{r build_corpus, message=FALSE, warning=FALSE}
corpus_news <- corpus(news[1:10000])
corpus_twitter <- corpus(twitter[1:10000])
corpus_blogs <- corpus(blogs[1:10000])
```
...
Via the `dfm` function we also create document feature matrices for bi- and tri-grams:
```{r build_matrix_ngram}
dfm_news_bi <- dfm(corpus_news, ngram=2)
dfm_twitter_bi <- dfm(corpus_twitter, ngram=2)
dfm_blogs_bi <- dfm(corpus_blogs, ngram=2)

dfm_news_tri <- dfm(corpus_news, ngram=3)
dfm_twitter_tri <- dfm(corpus_twitter, ngram=3)
dfm_blogs_tri <- dfm(corpus_blogs, ngram=3)
```

```{r clean_corp, echo=FALSE, message=FALSE}
rm(corpus_news)
rm(corpus_twitter)
rm(corpus_blogs)
```
